...
public struct Worktree: Sendable, Equatable {
    public let path: URL
    /// The checked-out branch name (short form, e.g. `main`, `feature/x`). `nil` for a detached HEAD.
...
/// Branch information obtained from `for-each-ref`.
public struct Branch: Sendable, Equatable {
    ...
    /// For local, the short branch name (e.g. `main`); for remote, the `<remote>/<branch>` form (e.g. `origin/main`).
...
/// Result of `git diff --shortstat` plus the working tree's dirty check.
...
/// Source pattern for `addWorktree`.
public enum WorktreeSource: Sendable, Equatable {
    ...
    /// If `startPoint` is omitted, the branch diverges from the current HEAD.
    ...
    /// If `newLocalName` is omitted, the local branch gets the same name as the remote one.
    case remoteBranch(remote: String, name: String, newLocalName: String? = nil)
}

/// Merge method between worktrees.
public enum MergeStrategy: Sendable, Equatable {
    /// Run `git merge <arguments> <source>` in the target's worktree. Default is `--no-ff`.
    case merge(arguments: [String] = ["--no-ff"])
    /// Run `git rebase <target>` in the source's worktree, then
    /// `git merge --ff-only <source>` in the target's worktree.
    case rebase
}

/// Errors specific to GitService operations (not plain git command failures).
