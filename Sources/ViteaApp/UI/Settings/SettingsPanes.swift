...
final class WorktreeSettingsPane: SettingsPane {
    ...
    override func buildForm() {
        ...
        setFootnote("プレースホルダ: {project}(リポジトリ名)、{branch}(/ は - に正規化)、{branch_raw}(そのまま)。フックには VITEA_WORKTREE_PATH / VITEA_BRANCH / VITEA_GIT_ROOT が渡されます。")
    }
...
final class HooksSettingsPane: SettingsPane {
    override func buildForm() {
        ...
        setFootnote("セッション状態の変化時に /bin/sh -c で実行されます(空で無効)。環境変数: VITEA_SESSION_NAME / VITEA_WORKTREE_PATH / VITEA_OLD_STATE / VITEA_NEW_STATE。例: 入力待ちで音を鳴らす → afplay /System/Library/Sounds/Glass.aiff")
    }
