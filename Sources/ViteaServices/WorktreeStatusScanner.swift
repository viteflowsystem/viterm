...
/// サイドバー表示用の `ViteaCore.Worktree` に変換する。
///
/// `GitKit.Worktree` と `ViteaCore.Worktree` は同名のため、このファイル内では常にモジュール修飾で区別する。
public struct WorktreeStatusScanner: Sendable {
    ...
    public func scan(repositories: [ViteaCore.Repository]) async -> [ViteaCore.Worktree] {
        var result: [ViteaCore.Worktree] = []
    ...
    public func scan(repository: ViteaCore.Repository) async throws -> [ViteaCore.Worktree] {
        ...
        var results: [ViteaCore.Worktree] = []
