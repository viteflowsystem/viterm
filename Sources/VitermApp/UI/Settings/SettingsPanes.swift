...
// The settings window's panes. The shared SettingsPane base provides form-building
// helpers (adding grid rows, immediate saving of changes) so each pane can focus on its
// field definitions. To add a pane, just write a SettingsPane subclass and add one line
// to SettingsWindowController.panes.
...
class SettingsPane: NSViewController, NSTextFieldDelegate {
    ...
    override func loadView() {
        ...
        // Notes align with the start of the field column.
        ...
        NSLayoutConstraint.activate([
            ...
            // The window height comes from fittingSize, so pin bottom with equal
            // (lessThanOrEqual would let the height collapse to 0 and hide the window).
            stack.bottomAnchor.constraint(equalTo: container.bottomAnchor),
            container.widthAnchor.constraint(equalToConstant: Layout.contentWidth),
        ])
        view = container
        buildForm()
        // Columns are created as rows are added, so configuring before buildForm raises an out-of-range exception.
        ...
        // Left unset, the initial window size remains and leaves a huge empty margin.
    ...
    /// Place in the right column only, with no label (checkboxes, notes).
    ...
    func makeTextField(value: String, placeholder: String = "", onCommit: @escaping (String) -> Void) -> NSTextField {
        ...
        // The same field width across all panes (non-stretching; pinned equal for consistency).
        field.widthAnchor.constraint(equalToConstant: Layout.fieldWidth).isActive = true
        return field
    }

    /// Vertical stack placing an annotation (preview, etc.) directly beneath a field. Used as a row's field.
    ...
    /// Fixed width for popups and the like (aligning start position and look with text fields).
    func fixWidth(_ view: NSView, _ width: CGFloat = 200) {
        view.widthAnchor.constraint(equalToConstant: width).isActive = true
    }
}

/// Text field that calls onCommit only on focus loss or Enter confirmation.
final class CommitTextField: NSTextField, NSTextFieldDelegate {
    var onCommit: ((String) -> Void)?
    /// Called on every change (before confirmation). For preview updates.
