...
enum Fixture {
    ...
    /// Create a set of repositories with a bare remote ("origin"). `repo` is a clone with
    /// origin/HEAD = main, and the `feature` branch is already pushed to origin and fetched
    /// into `repo`.
    /// - Returns: (repo: the working clone, bare: path of the bare remote)
