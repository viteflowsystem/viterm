...
// MARK: - Parsing (pure logic, no git needed)
...
@Test func gitRunner_timesOutOnHangingProcess() async throws {
    try await withTemporaryDirectory { dir in
        ...
        // Replacing via exec makes sleep a direct child of Process, so terminate() reliably works.
        let script = "#!/bin/sh\nexec sleep 5\n"
...
@Test func addWorktree_newBranch_createsWorktreeAndLocalBranch() async throws {
    try await withTemporaryDirectory { dir in
        ...
        // Also covers the case of becoming a subdirectory, like the {branch_raw} template.
...
@Test func removeWorktree_dirty_throwsWithoutForce() async throws {
    try await withTemporaryDirectory { dir in
        ...
        // No force, so the worktree still remains.
        let worktrees = try await service.worktrees(in: repo)
        #expect(worktrees.count == 2)
    }
}
...
@Test func merge_rebase_thenFastForwardsTarget() async throws {
    try await withTemporaryDirectory { dir in
        ...
        // Advance only main so feature stays based on the old main.
...
// MARK: - parseWorkingState (pure logic, no git needed)
