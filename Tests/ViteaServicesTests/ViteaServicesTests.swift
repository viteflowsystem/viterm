...
@Test func createWorktree_postCreationHook_runsAsynchronouslyWithExpectedEnvironment() async throws {
    try await withTemporaryDirectory { dir in
        ...
        #expect(invocations[0].environment["VITEA_WORKTREE_PATH"] == result.worktreePath)
        #expect(invocations[0].environment["VITEA_BRANCH"] == "feature-x")
        #expect(invocations[0].environment["VITEA_GIT_ROOT"] == repo.path)
    }
}
