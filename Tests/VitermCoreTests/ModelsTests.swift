...
struct ModelsTests {
    ...
    func repositoryRoundTrip() throws {
        let original = Repository(name: "viterm", path: "/repo/viterm")
        let data = try JSONEncoder().encode(original)
        let decoded = try JSONDecoder().decode(Repository.self, from: data)
        #expect(decoded == original)
        #expect(decoded.id == "/repo/viterm")
    }

    @Test("Worktree は Codable で往復でき、既定値は0/非dirty")
    func worktreeRoundTripAndDefaults() throws {
        let worktree = Worktree(path: "/repo/viterm/wt/feat-foo", repositoryPath: "/repo/viterm", branch: "feat/foo")
        ...
        let full = Worktree(
            path: "/repo/viterm/wt/feat-foo",
            repositoryPath: "/repo/viterm",
            branch: "feat/foo",
            ahead: 3,
            behind: 1,
    ...
    func agentSessionDefaultsAndRoundTrip() throws {
        let session = AgentSession(
            worktreePath: "/repo/viterm/wt/feat-foo",
            presetName: "claude",
