...
vitea の設定は2段階のファイルベースで、`~/.config/viterm/config.json`(グローバル)と
...
> `discoveryRoots` は `AppModel`(`Sources/ViteaServices/AppModel.swift`)側の配線(T9b)まで完了しており、
> 設定ファイルに書けばそのまま効く。
>
> `worktreePathTemplate` / `defaultPreset` / `copySessionDataByDefault` / `discoveryRoots` の4キーは
> `⌘,` の設定シート(`Sources/ViteaApp/UI/SettingsSheet.swift`)からも編集できる。保存先は常に
