...
viterm の設定は2段階のファイルベースで、`~/.config/viterm/config.json`(グローバル)と
`<リポジトリルート>/.viterm.json`(プロジェクト別)をマージして使う。
実装は `Sources/VitermCore/Config/`(`VitermConfig.swift` / `ConfigLoader.swift`)、
worktree 作成・状態変化・リポジトリ自動検出まわりの補助機能は `Sources/VitermServices/` にある。

> 本ドキュメントは実装済みのコードのみを典拠にしている。`postCreationHook` / `statusHooks` /
> `discoveryRoots` は `AppModel`(`Sources/VitermServices/AppModel.swift`)側の配線(T9b)まで完了しており、
> 設定ファイルに書けばそのまま効く。
>
> `worktreePathTemplate` / `defaultPreset` / `copySessionDataByDefault` / `discoveryRoots` の4キーは
> `⌘,` の設定シート(`Sources/VitermApp/UI/SettingsSheet.swift`)からも編集できる。保存先は常に
> グローバル設定(`~/.config/viterm/config.json`)で、既存 JSON を読み込んでこの4キーだけを差し替えて
> 書き戻すため、`presets` / `statusHooks` / `postCreationHook` など画面に無いキーは保全される。
> それ以外のキー(`presets` / `statusHooks` / `postCreationHook` / `repositories`)は現時点では
> `config.json` を直接編集する必要がある(設定シートに「config.json を開く」ボタンあり)。

## 1. ファイルの場所

| ファイル | パス | 用途 |
|---|---|---|
| グローバル設定 | `~/.config/viterm/config.json` | 全リポジトリ共通の既定値 |
| プロジェクト設定 | `<リポジトリルート>/.viterm.json` | そのリポジトリだけの上書き(dotfiles として管理可能) |

どちらのファイルも存在しなくてよい。両方無い場合は組み込みの既定値(`VitermConfig.default`)で動作する。
...
1つの `VitermConfig` にまとめる。

## 2. キー一覧

| キー | 型 | 既定値 | 説明 |
|---|---|---|---|
| `worktreePathTemplate` | string | `"~/worktrees/{project}/{branch}"` | worktree 作成先パスのテンプレート(§4参照)。**`⌘,` 設定シートから編集可** |
| `presets` | array | 下記「既定プリセット」参照 | セッション起動コマンドのプリセット一覧(§5参照) |
| `defaultPreset` | string \| null | `"shell"` | 新規セッション起動時に既定で選ぶプリセット名。**`⌘,` 設定シートから編集可**(選択肢は現在ロード中の `presets` の一覧) |
| `repositories` | array | `[]` | 登録済みリポジトリ一覧(`{ "name": string, "path": string }`) |
| `copySessionDataByDefault` | boolean | `false` | worktree 作成ダイアログで「Claude セッションデータをコピー」チェックボックスの初期値。**`⌘,` 設定シートから編集可** |
...
| `discoveryRoots` | array of string | `[]` | リポジトリ自動検出の走査ルートディレクトリ一覧(§9参照。グローバル設定のみ有効)。**`⌘,` 設定シートから編集可**(カンマ区切りテキストとして) |

> `postCreationHook` / `statusHooks` / `discoveryRoots` は config のスキーマ・マージ規則に加え、
> `AppModel.refresh()` でのランタイム反映(T9b)まで実装済み。設定ファイルに書けばそのまま効く。
...
```json
{
  ...
  "postCreationHook": "echo \"worktree ready: $VITERM_WORKTREE_PATH\"",
  "statusHooks": {
    "onWaitingInput": "terminal-notifier -message \"$VITERM_SESSION_NAME が入力待ちです\""
  },
...
`VitermConfig.merge(global:project:)` は「組み込み既定値 → グローバル → プロジェクト」の順に重ね合わせる。
...
- **`discoveryRoots`**: グローバル設定の値のみを使う(`global ?? default`)。`.viterm.json` に書いても
...
`presets` について具体的には、`.viterm.json` に `claude` プリセットを1つだけ書いても、組み込み既定の
...
`WorktreePathTemplate`(`Sources/VitermCore/WorktreePathTemplate.swift`)が `worktreePathTemplate` 文字列を
実際のパスへ展開する。
...
以下はホームディレクトリ `/Users/me`、project(リポジトリ名)`viterm`、
repositoryRoot(リポジトリルート)`/Users/me/dev/viterm` での展開結果。

| テンプレート | branch | 展開結果 |
|---|---|---|
| `~/worktrees/{project}/{branch}`(既定) | `feat/login` | `/Users/me/worktrees/viterm/feat-login` |
| `worktrees/{branch_raw}`(相対パス) | `feat/login` | `/Users/me/dev/viterm/worktrees/feat/login`(サブディレクトリになる) |
...
展開結果にも文字通り `/…/viterm/../feat-login` の形で `..` が残る(ファイルシステム上は正しく解決されるが、
プレビュー表示上はそのまま見える)。

## 5. プリセット(`presets`)

各プリセットは `SessionPreset`(`Sources/VitermCore/Models/SessionPreset.swift`)。
...
### 組み込み既定プリセット(`VitermConfig.defaultPresets`)
...
`defaultPreset` の既定値は `"shell"`(エージェントはシェル内でユーザーが起動する想定。常に claude を開きたい場合は `"claude"` に変更)。

## 6. リポジトリ一覧(`repositories`)

`Repository`(`Sources/VitermCore/Models/Repository.swift`)は `name`(サイドバー表示名。`{project}` にも使われる)と
`path`(リポジトリルートの絶対パス。これが一意キーになる)の2フィールドのみ。ディスク上のリポジトリ自体には
一切手を加えない、単なる参照情報。

## 7. hook の環境変数

`postCreationHook` / `statusHooks` は `AppModel` が `refresh()` のたびに読み直し、ランタイムに反映する。

- post-creation hook: `AppModel.createWorktree(from:)` で、フォーム(`NewWorktreeRequest.runHookCommand`)に
  hook が指定されていれば(空文字でない場合)そちらを優先し、指定が無ければ `config.postCreationHook` を
  フォールバックとして `WorktreeCreationRequest.postCreationHookCommand` に渡す。
- 状態変化 hook: `AppModel.refresh()` のたびに `config.statusHooks` から `StatusChangeHookConfig` を
  組み立て、`StatusChangeNotifying.updateConfig(_:)` で `StatusChangeHookRunner` に反映する
  (設定ファイルを書き換えて再起動 or 手動 `refresh()` すれば新しい hook が効く)。
...
| 環境変数 | 内容 |
|---|---|
| `VITERM_WORKTREE_PATH` | 作成された worktree の絶対パス |
| `VITERM_BRANCH` | チェックアウトされたブランチ名(生の形。`/` を含みうる) |
| `VITERM_GIT_ROOT` | リポジトリルートの絶対パス |
...
| 環境変数 | 内容 |
|---|---|
| `VITERM_SESSION_NAME` | セッションの表示名 |
| `VITERM_WORKTREE_PATH` | セッションが紐づく worktree の絶対パス |
| `VITERM_OLD_STATE` | 変化前の状態(`busy`/`waitingInput`/`idle`)。不明な場合は空文字列 |
| `VITERM_NEW_STATE` | 変化後の状態(`busy`/`waitingInput`/`idle`) |
...
`RepositoryDiscovery`(`Sources/VitermServices/RepositoryDiscovery.swift`)がルートディレクトリ配下の
git リポジトリを走査する(ccmanager の `CCMANAGER_MULTI_PROJECT_ROOT` 相当)。走査ルートは config の
`discoveryRoots`(グローバル設定のみ有効。§3参照)に文字列配列として書き、`AppModel.refresh()` が
各エントリを(`~` を実際のホームディレクトリに展開したうえで)それぞれ `RepositoryDiscovering.discover(rootDirectory:)`
に渡し、全ルートの検出結果を合算する。`discoveryRoots` が空配列(既定)の場合は自動検出自体を行わない。
...
検出結果は `VitermCore.Repository` の配列で返る(グローバル設定の `repositories` へ追加するかどうかは
