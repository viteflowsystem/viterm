...
> 執筆時点でまだ実装途上だが、`postCreationHook` / `statusHooks` / `discoveryRoots` は
> `AppModel`(`Sources/ViteaServices/AppModel.swift`)側の配線(T9b)は完了している。
> UI からの入力(worktree作成ダイアログの hook 入力欄、設定画面等)が config の値を上書きする経路は
> まだ無いため、現時点では常に config の値がそのまま使われる。
...
| `defaultPreset` | string \| null | `"shell"` | 新規セッション起動時に既定で選ぶプリセット名 |
