// Script that deterministically renders the viterm AppIcon with Core Graphics.
...
// Output: via iconutil, all sizes
//
// Design (on a 256pt canvas):
// - Follows the macOS icon grid: 824px rounded square within 1024px (corner radius 185px) + transparent margin
// - Background: navy #1E2442 (viteflow system brand)
// - ❯ prompt: stroke in the brand gradient #FF8A47→#FFBE3D→#FF7A4C→#FF3B8E
...
/// Draw the icon onto a 1024px canvas.
func draw(in ctx: CGContext) {
    ...
    // From here on, draw in the 256pt design coordinate system (top-origin).
    // Map 256pt onto the plate area (824px) so everything stays within the plate.
    ctx.saveGState()
    ...
    // DEV badge: amber ribbon + "DEV" at the bottom of the plate (recognizable even at small Dock sizes)
    if isDev {
